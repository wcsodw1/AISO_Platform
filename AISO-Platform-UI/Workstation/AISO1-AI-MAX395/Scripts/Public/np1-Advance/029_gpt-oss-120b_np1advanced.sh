#!/bin/bash
# ============================================
# AMD AI Max+ 395 - GPT-OSS-120B (Q4) Advanced ctx 測試
...
MODEL_FILE=$(find "$MODEL_DIR" -iname "*00001-of*.gguf" | head -1)
LOG_DIR="$MODEL_DIR/logs"
RESULT_CSV="$MODEL_DIR/029_gpt-oss-120b_np1advanced_results.csv"
PORT=8080
ALIAS="gpt-oss-120b-advanced"
...
echo "[開始] 腳本 029 (Advanced ctx) 開始執行時間: $(date '+%Y-%m-%d %H:%M:%S')"
...
for ctx in "${CTX_LIST[@]}"; do
  ...
  ACTUAL_CTX="NA"
  LOGFILE="$LOG_DIR/029_ctx${ctx}_$(date +%H%M%S).log"
