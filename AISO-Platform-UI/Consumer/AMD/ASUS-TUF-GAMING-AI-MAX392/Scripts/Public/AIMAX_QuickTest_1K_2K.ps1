<#
.SYNOPSIS
  Quick llama.cpp Vulkan NP1 benchmark for Ryzen AI MAX+ 392.

.DESCRIPTION
  Quick validation for actual 1K and 2K input tokens, 300 output tokens,
  NP=1, three trials, and the same four formal metrics as the full benchmark.
  ...
  2. Run / RunServer (RunServer is default)
     - Captures OS, CPU, memory, GPU driver, power plan, llama.cpp build, and
       Vulkan device information.
     - Runs the real llama-server NP1 API matrix for 1K and 2K inputs.
     - Produces raw JSON/stderr logs, environment.json, model_hashes.csv,
       benchmark_trials.csv, benchmark_summary.csv, and benchmark_report.md.
...
.RUNNING COMMANDS
  Prepare llama.cpp and models:
    powershell -NoProfile -ExecutionPolicy Bypass -File .\AIMAX_QuickTest_1K_2K.ps1 -Action Prepare

  Run the benchmark:
    powershell -NoProfile -ExecutionPolicy Bypass -File .\AIMAX_QuickTest_1K_2K.ps1 -Action Run

  Run the formal API suite only:
    powershell -NoProfile -ExecutionPolicy Bypass -File .\AIMAX_QuickTest_1K_2K.ps1 -Action RunServer

  Compare after copying the other machine's latest_summary.csv locally:
    powershell -NoProfile -ExecutionPolicy Bypass -File .\AIMAX_QuickTest_1K_2K.ps1 -Action Compare -CompareCsv "D:\Transfer\MAX395_latest_summary.csv"
...
function Convert-ServerTrials {
    ...
    foreach ($inputSize in $InputTokenSizes) {
        ...
        $summary += [PSCustomObject][ordered]@{
            ...
            TPOT_ms_per_token = [math]::Round((Get-Mean $tpot), 6)
...
function Invoke-LlamaServerSuite {
    ...
    foreach ($model in $Models) {
        ...
        if ($failures.Count -gt 0) {
            $failures | Export-Csv -Path (Join-Path $RunDir ($safeName + "_server_failures.csv")) -NoTypeInformation -Encoding UTF8
        }
    }

    if ($allTrials.Count -gt 0) {
        $allTrials | Select-Object Machine, ModelName, ModelFile, InputTokens,
            OutputTokensRequested, ContextSize, Trial,
            @{ Name = "TTFT_ms"; Expression = { $_.TTFTms } },
            @{ Name = "TPOT_ms_per_token"; Expression = { $_.TPOTms } },
            @{ Name = "Throughput_Aggregate_tokens_per_s"; Expression = { $_.AggregateThroughput } },
            @{ Name = "Throughput_Normalized_tokens_per_s_per_user"; Expression = { $_.NormalizedThroughput } } |
            Export-Csv -Path (Join-Path $RunDir "benchmark_trials.csv") -NoTypeInformation -Encoding UTF8
    }
    return $summary
}
function New-BenchmarkReport {
    ...
    [void]$lines.Add("- Throughput Aggregate (tokens/s): total output tokens across the measured requests / total wall-clock request duration; higher is better.")
    ...
    [void]$lines.Add("")
    [void]$lines.Add("Each row is one model/input test. Values are aggregated from the completed trials shown in the Trials column.")
    [void]$lines.Add("")
    [void]$lines.Add("| Model | Input | Output | Context | Trials | TTFT (ms) | TPOT (ms/token) | Throughput Aggregate (tokens/s) | Throughput Normalized (tokens/s/user) |")
    [void]$lines.Add("|---|---:|---:|---:|---:|---:|---:|---:|---:|")
    foreach ($row in $formalRows) {
        [void]$lines.Add("| $($row.ModelName) | $($row.InputTokens) | $($row.OutputTokens) | $($row.ContextSize) | $($row.Trials) | $($row.TTFT_ms) | $($row.TPOT_ms_per_token) | $($row.Throughput_Aggregate_tokens_per_s) | $($row.Throughput_Normalized_tokens_per_s_per_user) |")
    }
    [void]$lines.Add("")
    [void]$lines.Add("## Validity checks")
    [void]$lines.Add("")
    [void]$lines.Add("1. Compare only the same model, input length, output length, server context, and NP.")
    ...
    [void]$lines.Add("4. Match AC power, Windows power mode, AMD driver, BIOS/UMA, and cooling.")
    [void]$lines.Add("5. Disclose RAM capacity/speed and unavoidable configuration differences.")

    $failureFiles = @(Get-ChildItem -LiteralPath (Split-Path -Parent $Path) -Filter "*_server_failures.csv" -File -ErrorAction SilentlyContinue)
...
function Invoke-BenchmarkRun {
    ...
    $trialsPath = Join-Path $runDir "benchmark_trials.csv"
    ...
    if (Test-Path -LiteralPath $trialsPath) {
        Copy-Item -LiteralPath $trialsPath -Destination (Join-Path $ResultsRoot "latest_trials.csv") -Force
    }
