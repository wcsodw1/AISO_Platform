<#
.SYNOPSIS
  Ryzen AI MAX+ 395 llama.cpp Vulkan rerun with matched context and Performance power mode.
...
.DESCRIPTION
  ...
  Test matrix (fixed on both machines):
    ...
    AMD driver       unchanged for this rerun; current version is logged only
    VGM/Vulkan memory unchanged for this rerun; current runtime state is logged only
...
.RUNNING COMMANDS
  Prepare llama.cpp and models:
    powershell -NoProfile -ExecutionPolicy Bypass -File .\V5_0828_AIMAX395_1np_Benchmark.ps1 -Action Prepare

  Preflight controlled settings:
    powershell -NoProfile -ExecutionPolicy Bypass -File .\V5_0828_AIMAX395_1np_Benchmark.ps1 -Action Preflight

  Run the benchmark:
    powershell -NoProfile -ExecutionPolicy Bypass -File .\V5_0828_AIMAX395_1np_Benchmark.ps1 -Action Run

  Run the formal API suite only:
    powershell -NoProfile -ExecutionPolicy Bypass -File .\V5_0828_AIMAX395_1np_Benchmark.ps1 -Action RunServer

  Compare after copying the other machine's latest_summary.csv locally:
    powershell -NoProfile -ExecutionPolicy Bypass -File .\V5_0828_AIMAX395_1np_Benchmark.ps1 -Action Compare -CompareCsv "D:\Transfer\MAX392_latest_summary.csv"
...
[CmdletBinding()]
param(
    [ValidateSet("Prepare", "Preflight", "Run", "RunServer", "Compare")]
    [string]$Action = "Run",

    [string]$CompareCsv = "",

    # Windows High performance is enforced automatically for Run/RunServer/Preflight.
    # Driver and VGM/Vulkan memory are intentionally NOT changed or gated in this rerun.
    # OEM Silent/Performance mode is separate and must still be set to Performance manually.
...
$Root = "C:\AIMAX395_Benchmark"
$MachineLabel = "Ryzen AI MAX+ 395"
...
# Controlled-comparison policy for this rerun:
# - Match the 392 context policy.
# - Force Windows High performance.
# - Keep each machine at its current driver and VGM/Vulkan-memory configuration.
...
function Set-WindowsBenchmarkPowerPlan {
    ...
    Write-Warning "OEM thermal/performance mode is independent of Windows powercfg. Before the benchmark, set the machine vendor's mode to Performance/Turbo, not Silent."
}

function Get-AmdDisplayDriverVersion {
    $amdGpu = Get-CimInstance Win32_VideoController | Where-Object {
        $_.Name -match "AMD|Radeon|8060S"
    } | Select-Object -First 1

    if ($null -eq $amdGpu) { return "" }
    return [string]$amdGpu.DriverVersion
}

function Write-ControlledBenchmarkPreflight {
    Write-Step "Context + power rerun preflight"
    $computer = Get-CimInstance Win32_ComputerSystem
    $activePower = ((powercfg /GETACTIVESCHEME 2>&1 | Out-String).Trim())
    $driver = Get-AmdDisplayDriverVersion

    Write-Host "Machine             : $MachineLabel"
    Write-Host "Threads             : $Threads (all logical processors)"
    Write-Host "Context policy      : $(Get-ContextPolicyText)"
    Write-Host "Windows power plan  : $activePower"
    Write-Host "AMD driver          : $driver (UNCHANGED; logged only)"
    Write-Host "Physical memory     : $([math]::Round($computer.TotalPhysicalMemory / 1GB, 2)) GiB"
    Write-Host "Driver / VGM        : unchanged for this rerun"
    Write-Warning "OEM thermal/performance mode is independent of Windows powercfg. Set the machine vendor's mode to Performance/Turbo, not Silent. Driver and VGM/Vulkan memory are intentionally left untouched."

    if (Test-Path -LiteralPath $LlamaCli) {
        Write-Host "`nllama.cpp Vulkan device report (observation only; no setting is changed):"
        Write-Host (Invoke-NativeCapture -FilePath $LlamaCli -Arguments @("--list-devices"))
    }
}
...
function Get-EnvironmentRecord {
    ...
    return [PSCustomObject][ordered]@{
        ...
        AmdDriverVersion = (Get-AmdDisplayDriverVersion)
...
function New-BenchmarkReport {
    ...
    [void]$lines.Add("| AMD driver | $($Environment.AmdDriverVersion) (unchanged for this rerun) |")
...
function Invoke-BenchmarkRun {
    ...
    Write-ControlledBenchmarkPreflight
    ...
    $environment | ConvertTo-Json -Depth 6 | Set-Content -LiteralPath (Join-Path $runDir "environment.json") -Encoding UTF8
...
PREFLIGHT
powershell -NoProfile -ExecutionPolicy Bypass -File "$PSCommandPath" -Action Preflight
...
switch ($Action) {
    ...
    "Preflight" {
        ...
        Write-ControlledBenchmarkPreflight
        Assert-BenchmarkReady
        Write-Host "`nPreflight passed. Confirm OEM mode is Performance/Turbo. Driver and VGM/Vulkan memory remain unchanged for this rerun." -ForegroundColor Green
    }
    "Run" {
        Invoke-BenchmarkRun
    }
